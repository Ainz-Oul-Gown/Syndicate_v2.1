...
serve(async (req) => {
    ...
    try {
        ...
        const { data: recipientUsers } = await admin.from('users').select('id, tg_id').in('tg_id', recipientIds)
        const filteredUsers = (recipientUsers || []).filter((u: any) => u.tg_id !== senderId)
        const filteredUuids = filteredUsers.map((u: any) => u.id)

        if (!filteredUuids.length) return json({ ok: true, sent: 0 })

        const { data: subscriptions, error } = await admin.from('push_subscriptions')
            .select('id,token').in('user_id', filteredUuids).eq('active', true)
        ...
        for (const subscription of subscriptions) {
            const response = await fetch(`https://fcm.googleapis.com/v1/projects/${serviceAccount.project_id}/messages:send`, {
                ...
                body: JSON.stringify({
                    message: {
                        token: subscription.token, data: {
                            title, body, tag: `chat-${chatId}`, chatId: String(chatId), url: `/?chat=${encodeURIComponent(chatId)}`,
