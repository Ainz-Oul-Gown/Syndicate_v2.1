...
serve(async (req) => {
  ...
  try {
    ...
    const ipOk = await checkRateLimit(supabaseAdmin, `seed_challenge:${clientIp}`, 'challenge_ip', 10, 10)
    if (!ipOk) {
      await recordAuthAttempt(supabaseAdmin, `seed_challenge:${clientIp}`, 'challenge_ip', false)
      throw new Error('Не удалось подготовить запрос')
    }

    // В1: Rate-limit по stableId — не более 5 запросов challenge за 10 минут
    // Защита от DoS через overwrite challenge (атакующий перезаписывает challenge жертвы)
    const stableIdOk = await checkRateLimit(supabaseAdmin, `seed_challenge:${stableId}`, 'challenge_stable', 5, 10)
    if (!stableIdOk) {
      await recordAuthAttempt(supabaseAdmin, `seed_challenge:${stableId}`, 'challenge_stable', false)
      throw new Error('Не удалось подготовить запрос')
    }

    await recordAuthAttempt(supabaseAdmin, `seed_challenge:${clientIp}`, 'challenge_ip', true)
    ...
    return json({ challenge, expiresAt }, 200, origin);
  } catch (err: any) {
